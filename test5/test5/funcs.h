...
#define TRUE 1
#define FALSE 0
#define MAX_LITE_NUM 2047 //SAT问题的最大文字数，当前为11bit
...
typedef struct Node {
	literal index:11; //文字在文字列表的索引，当结点作为子句头结点时值为-1
...
typedef struct {
	...
	unsigned int literalNum : 11; //文字数
	clauseHead *chainHead; //首个子句的头指针
} root; //子句“网”的头结点

typedef struct {
	unsigned int shortestLen : 11; //该文字出现的最短子句的长度
...
typedef struct levelNode {
	unsigned int level : 11; //被标记的层数，即>=level可认为此节点不存在，0为未被标记
	literal index:11; //文字在文字列表的索引，当结点作为子句头结点时值为-1
	unsigned int value : 1; //文字在此句中的正反，TRUE为正，FALSE为反
...
typedef struct clausHead2 {
	//TODO: length的位字段为宏MAX_LITE_NUM所占位数
	unsigned int length : 11; //子句长度，当结点作为子句链头结点时值为-1
